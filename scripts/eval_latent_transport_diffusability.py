...
def estimate_decoder_spectral_gain(
    *,
    ...
    anchors = torch.randn((num_anchors, 4, 32, 32), generator=generator)
    amp_enabled = bool(amp and device.type == "cuda")

    print(
        f"decoder_gain_start model={model} device={device} anchors={num_anchors} "
        f"batch_size={batch_size} probes_per_band={probes_per_band} epsilon={epsilon} amp={amp_enabled}",
        flush=True,
    )
    with torch.no_grad():
    ...
    _write_csv(model_dir / "decoder_spectral_gain_summary.csv", [_gain_summary_row(model, vae_dir, gain_np, args=None)])
    _plot_gain_heatmap(model_dir, model, gain_np)
    print(f"decoder_gain_done model={model} output={model_dir}", flush=True)


def _gain_summary_row(model: str, vae_dir: str, gain: np.ndarray, args) -> dict[str, object]:
...
def analyze_latent_transport(args: argparse.Namespace) -> None:
    ...
    for model in args.models:
        ...
        summary_rows.append(_gain_summary_row(model, _vae_dir_for_model(args, model), gain, args))
