...
class LinearVPSchedule:
    """Continuous linear-beta VP schedule used by the official U-ViT ImageNet setup."""

    def __init__(self, beta_min: float = 0.1, beta_max: float = 20.0, eps: float = 1e-4):
    ...
    def alpha_bar(self, t: torch.Tensor) -> torch.Tensor:
        t = t.clamp(0.0, 1.0)
        return (-self._integral_beta(t)).exp().clamp(min=1e-12, max=1.0)
